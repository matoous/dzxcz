...
FROM activities.csv WHERE activity_date LIKE '%s'`, dateQuery)
	if len(optSportType) > 0 {
		sports := make([]string, 0, len(optSportType))
		for _, st := range optSportType {
			sports = append(sports, fmt.Sprintf("activity_type == '%s'", st))
		}
		query += fmt.Sprintf(" AND (%s)", strings.Join(sports, " OR "))
	}
	fmt.Println(query)
...
func main() {
	...
	running.Unit = "runs"
	cycling, err := sportStats(asOf, "Ride", "Virtual Ride")
	if err != nil {
		panic(err)
	}
	cycling.Unit = "rides"
